function effective_resistance(g, D = incidence_matrix(g, oriented = true)')
    L_pinv = pinv(full(laplacian_matrix(g)));
    m = size(D,1); out = zeros(m);
    for i = 1:m
...
# a function for BGSM_effective_resistance (Section 2,3)
...
function BGSM_effective_resistance(
                          y, D, R;                              # a 
    ...
    for i = 1:iter
        ...
        q = 1./( 1 + (1-eta)/eta * sqrt(v0/v1).^R .* exp.(delta.^2/2 * (1/v0 - 1/v1)/sigmasq) );
        q = min.(max.(q,0),1)
        tau = q/v0 + (1-q)/v1;
